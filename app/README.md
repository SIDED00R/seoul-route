...
| 다른 앱 위에 표시 | 다른 앱 사용 중 미니 지도·안내 표시(선택). 본체는 터치를 통과시키고 진하기 손잡이만 터치를 받음 |

알림·활동 인식·다른 앱 위에 표시 권한을 거부해도 안내는 계속되지만 관련 표시·필터가 빠집니다. 위치 권한이나 위치 서비스가 없으면 안내를 시작할 수 없습니다. 오버레이는 설정에서 켜고 저장하며(안내 중이면 그 안내에 바로 반영), 안내 중 앱을 떠나면 나타나고 앱으로 돌아오면 자동으로 숨습니다. 오른쪽 위 진하기 손잡이(32dp, 펼치면 슬라이더 띠)를 뺀 오버레이 본문은 터치를 통과시켜 아래 앱을 그대로 조작할 수 있습니다.
...
4. 오버레이를 켠 경우 홈으로 이동해 미니 지도 표시·터치 통과·앱 복귀 시 숨김 확인
5. 화면을 끄거나 홈으로 이동한 뒤에도 샘플 업로드 확인
6. 안내 종료 후 남은 샘플 전송과 속도 프로파일 확인
7. `flutter run`과 `adb logcat -s flutter:E AndroidRuntime:E`에서 예외 확인
