...
| 다른 앱 위에 표시 | 다른 앱 사용 중 터치가 통과하는 미니 지도·안내 표시(선택) |

알림·활동 인식·다른 앱 위에 표시 권한을 거부해도 안내는 계속되지만 관련 표시·필터가 빠집니다. 위치 권한이나 위치 서비스가 없으면 안내를 시작할 수 없습니다. 오버레이는 설정에서 직접 켜며, 안내 중 앱을 떠나면 나타나고 앱으로 돌아오면 자동으로 숨습니다. 오버레이 자체는 터치를 받지 않아 아래 앱을 그대로 조작할 수 있습니다.
