...
| T-data 신호 잔여시간 `v2xSignalPhaseTimingInformation/1.0` | ⚠️ 2026-09-12 14:27~14:28 호출 3회 모두 **서버 500**(T-data DB 장애). **2026-09-15 재실측(이슈 #48)**: 서버 복구, 10행·1,000행 200. **같은 API 5분에 1회 제한** — 연달아 부르면 `429 V2X_REPEAT_CALL_LIMIT`(`limitSeconds 300`). 15:51 에 받은 1,000행이 **전부 교차로 1곳**(`itstId 1850`)의 약 1.2초 간격 로그였고 시각은 15:00:00~15:20:17 로 호출 시점보다 30분 넘게 뒤처졌다. 방향별 보행신호 잔여시간(`ntPdsgRmdrCs` 등, 1/10초, 이 표본 0.3~132.9초) 필드는 있고 **교차로 좌표 필드는 없다**. 교차로 지도 API 이름 후보(`v2xCrossroadMapInformation/1.0` 404, `v2xMapInformation/1.0` 등 410)는 못 찾았다 | 경로 요청마다 잔여시간을 조회하는 방식은 불가(아래 게이트 b). 엔드포인트 `https://t-data.seoul.go.kr/apig/apiman-gateway/tapi/v2xSignalPhaseTimingInformation/1.0?apikey=…&numOfRows=&pageNo=` |
...
- (b) T-data 매핑 가능 여부: **실시간 반영 불가(2026-09-15 판정, 이슈 #48)**. 09-12 는 서버 500 이었고, 09-15 복구 후 실측에서 같은 API 5분 1회 제한·한 페이지 1,000행이 교차로 1곳의 30분 넘게 지난 로그·교차로 좌표 없음을 확인했다. 경로 요청 때 도보 구간 교차로의 현재 잔여시간을 받는 계획 v2 방식은 쓸 수 없으므로 횡단보도 대기는 정적 기대값(`docs/crossing-wait.md`, 곳당 38초)을 유지한다. 교차로별 주기 추정에 쓸 여지는 있으나 `itstId`↔좌표 매핑 자료가 없어 보류.
