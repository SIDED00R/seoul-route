...
func TestBuildBusOnly(t *testing.T) {
	...
	rep, err := Build(out, []BusRoute{sampleRoute()}, nil, nil, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestBuildSplitsAtTurnaround(t *testing.T) {
	...
	rep, err := Build(out, []BusRoute{b}, nil, nil, nil, nil)
...
func TestBuildDropsStopsOutsideBBox(t *testing.T) {
	...
	if _, err := Build(out, []BusRoute{b}, nil, nil, nil, nil); err != nil {
		t.Fatal(err)
	}
...
func TestDwellSecByRouteType(t *testing.T) {
	...
	b := sampleRoute()
	b.Route.Type = "2"
	rep, err := Build(filepath.Join(t.TempDir(), "m.zip"), []BusRoute{b}, nil, nil, nil, nil)
...
func TestBuildSkipsRouteWithFirstEqualsLast(t *testing.T) {
	...
	out := filepath.Join(t.TempDir(), "g.zip")
	rep, err := Build(out, []BusRoute{r}, nil, nil, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
